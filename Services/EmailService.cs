...
namespace Talkable.Services
{
    ...
    {
        private string resetmessage { get; } = $@"
...
<div dir='rtl' style='font-family: Tahoma, Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px; padding: 20px; text-align: center;'>
    <h2 style='color: #673AB7;'>مرحباً بك في Talkable!</h2>
    <p style='font-size: 16px; color: #555;'>شكراً لانضمامك إلينا،</p>
    <p style='font-size: 14px; color: #777;'>يسعدنا جداً انضمامك لمنصة <strong>Talkable</strong>. يرجى استخدام رمز التحقق أدناه لتأكيد بريدك الإلكتروني وتفعيل حسابك:</p>
    
    <div style='background-color: #f9f9f9; padding: 15px; border-radius: 8px; margin: 20px 0;'>
        <p style='font-size: 12px; color: #999; margin-bottom: 5px;'>رمز تفعيل الحساب (OTP):</p>
        <h1 style='color: #673AB7; letter-spacing: 5px; margin: 0;'>Code</h1>
    </div>

    <p style='font-size: 13px; color: #888;'>هذا الكود صالح لمدة 10 دقائق.</p>
    <hr style='border: 0; border-top: 1px solid #eee; margin: 20px 0;'>
    <p style='font-size: 11px; color: #aaa;'>إذا لم تقم بإنشاء حساب في Talkable، يمكنك تجاهل هذا الإيميل.</p>
    <p style='font-size: 12px; color: #673AB7; font-weight: bold;'>فريق عمل Talkable</p>
</div>";
        public async Task SendEmailAsync(string to, string subject, string code)
        {
            ...
            if (subject == "Password Reset OTP")
            {
                ...
                {
                    Text = resetmessage.Replace("Code", code)
                };
            }
            else if (subject == "Email Confirmation OTP")
            {
